...
# Опционально: лимит размера загружаемого видео в МБ (по умолчанию 300)
# VIDEO_SIZE_LIMIT_MB=300
```
...
Скрипт `backend/migrate_db.js` добавляет колонку `fullDescription` в таблицу «Photo» (если её нет) и **создаёт таблицу «Video»** (если её нет). Это нужно, если база была создана до появления раздела «Видео» в проекте.

Миграцию удобнее выполнить после первого полного запуска (см. шаг 8.2).
...
Скрипт `migrate_db.js` создаёт таблицу **Video** (если её нет) и добавляет колонку **fullDescription** в таблицу Photo. Применяйте его, если разворачиваете обновлённую версию с разделом «Видео» на уже существующей базе:
...
### 8.3. Загрузка видео (настройка уже включена)

Конфигурация nginx в `deploy/nginx/frontend.conf` настроена на приём больших файлов: лимит тела запроса 350 МБ и увеличенные таймауты для загрузки видео. Если нужен другой лимит, задайте в `backend/.env` переменную `VIDEO_SIZE_LIMIT_MB` и при необходимости измените `client_max_body_size` в конфиге nginx (значение должно быть не меньше лимита на бэкенде).
...
3. Перейдите в галерею и в раздел «Видео» — контент появится после загрузки через админ-панель.
...
- [ ] Запущена БД, применён `init.sql` (при обновлении с уже существующей БД — при необходимости выполнен `migrate_db.js`).
